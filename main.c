...
static int  get_user_input(important_constants local_name_of_enum_value);  // Don't know if I need to put local_name_of_enum_value here     /* get a valid integer menu choice */   // ----   Validates user input for the menu too  - don't need to touch
...
//  declaring some arrays to make them global to avoid having to pass them as parameters between the main menu etc.
//char *array_of_input_labels = (char*) malloc(100 * sizeof(char));
char array_of_output_labels;
int array_of_input_combos;
int array_of_expected_outputs;
//  */

int main(void)
{
    menu_item_4(); // Running this function first to set all the labels and values for the created pins of the gates to zero
...
static void main_menu(void)
{
    ...
    important_constants cur_items = MENU_ITEMS;
    printf("\nSelect item: ");
...
static int get_user_input(important_constants local_name_of_enum_value)
{
    ...
    do {
        ...
        if (!is_integer(buf)) {
            ...
            else if (value >= 1 && value <= local_name_of_enum_value) { // for a choice between numbered menu options
                valid_input = 1;
            } else {
